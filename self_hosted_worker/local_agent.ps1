...
$WorkerStateFile = Join-Path $InstallRoot 'worker-process.json'
...
function Clear-WorkerState { try{Remove-Item $WorkerStateFile -Force -ErrorAction SilentlyContinue}catch{} }
function Save-WorkerState {
  if(-not $script:WorkerProcess){return}
  try{
    $state=@{pid=[int]$script:WorkerProcess.Id;startTimeUtcTicks=[long]$script:WorkerProcess.StartTime.ToUniversalTime().Ticks}|ConvertTo-Json -Compress
    Set-Content -Path $WorkerStateFile -Value $state -Encoding ASCII
  }catch{}
}
function Recover-WorkerState {
  if(-not(Test-Path $WorkerStateFile)){return}
  $proc=$null
  try{
    $state=Get-Content $WorkerStateFile -Raw -ErrorAction Stop|ConvertFrom-Json
    $savedPid=[int]$state.pid;$ticks=[long]$state.startTimeUtcTicks
    if($savedPid -le 0 -or $ticks -le 0){throw 'État worker local invalide.'}
    $proc=[System.Diagnostics.Process]::GetProcessById($savedPid)
    if($proc.HasExited -or [long]$proc.StartTime.ToUniversalTime().Ticks -ne $ticks){throw 'État worker local périmé.'}
    $script:WorkerProcess=$proc;$script:WorkerPid=$savedPid
  }catch{
    if($proc){try{$proc.Dispose()}catch{}}
    $script:WorkerProcess=$null;$script:WorkerPid=$null;Clear-WorkerState
  }
}
function Worker-IsRunning {
  if(-not $script:WorkerProcess){$script:WorkerPid=$null;Clear-WorkerState;return $false}
  try{
    if($script:WorkerProcess.HasExited){
      ...
      $script:WorkerProcess=$null;$script:WorkerPid=$null;Clear-WorkerState;return $false
    }
    return $true
  }catch{
    try{$script:WorkerProcess.Dispose()}catch{}
...
function Start-Worker([string]$studioUrl,[string]$studioToken) {
  ...
  $proc=[System.Diagnostics.Process]::Start($psi);if(-not $proc){throw 'Impossible de démarrer le worker.'};$script:WorkerProcess=$proc;$script:WorkerPid=$proc.Id;Save-WorkerState;Start-Sleep -Milliseconds 1700
