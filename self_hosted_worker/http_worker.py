...
RENEW_SECONDS=max(1800,min(6*3600,int(os.environ.get('WORKER_RENEW_SECONDS','10800'))))
...
def heartbeat_payload():
    return {'engine':ENGINE_VERSION,'profile':PROFILE_NAME,'resolution':[RENDER_WIDTH,RENDER_HEIGHT],'fps':RENDER_FPS,'ffmpegThreads':FFMPEG_THREADS,'localAI':bool(local_ai_enabled()),'model':MODEL if local_ai_enabled() else None}
...
def process_remote_job(job):
    ...
    with tempfile.TemporaryDirectory(prefix='autodirector_pc_') as td:
        ...
        brief={'engine':ENGINE_VERSION,'transport':'https-remote-worker','project':project_name,'targetDuration':target,'sourceCount':len(sources),'referenceCount':len(refs),'styleFingerprint':style,'contentProfile':profile,'performanceMemory':context.get('winningStrategies',[]),'localAI':local_ai_enabled(),'profileName':PROFILE_NAME}
        progress(jid,'director',22,'Director V8.7: simulation multi-plans',brief=brief)
        scores=[];revisions=0;last_strategy=''
        for variant in range(variants):
            if is_cancelled(jid):raise RuntimeError('JOB_CANCELLED')
            plan,simulations=choose_plan(project_name,sources,style,profile,context,target,variant,0);plan,local_diag=refine_plan(project_name,plan,sources,paths,work);last_strategy=plan['strategy']
            ...
            initial=work/f'AutoDirector_PC_{variant+1}.mp4';render_plan(work,plan,paths,initial,captions,voice);score,diag=critic(initial,target,plan);score,vlm_diag=critic_video(initial,score,plan,work);diag={**diag,'localVLM':vlm_diag};final=initial;revision_count=0
            if auto_revision and score<82 and MAX_REVISIONS>0:
                ...
                plan2,_=choose_plan(project_name,sources,style,profile,context,target,variant,1);plan2,local_diag2=refine_plan(project_name,plan2,sources,paths,work);revised=work/f'AutoDirector_PC_{variant+1}_R1.mp4';render_plan(work,plan2,paths,revised,captions,voice);score2,diag2=critic(revised,target,plan2);score2,vlm_diag2=critic_video(revised,score2,plan2,work);diag2={**diag2,'localVLM':vlm_diag2,'localAIDirector':local_diag2}
        ...
        with client(40) as c:
            r=c.post(f'/api/local-worker/jobs/{jid}/complete',json={'score':max(scores) if scores else 0,'revisionCount':revisions,'strategy':last_strategy,'message':'V8.7 PC terminé · galerie prête'});r.raise_for_status()
