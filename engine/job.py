...
def process_job(jid):
    ...
    try:
        ...
        update_job(jid,'running','analysis',6,'V9 Moment Ranker · analyse des rushs')
        context=load_context(project_id)
        with tempfile.TemporaryDirectory(prefix='autodirector_v9_') as td:
            ...
            brief={'engine':ENGINE_VERSION,'project':project[0] if project else 'Auto Director','targetDuration':target,'sourceCount':len(sources),'referenceCount':len(refs),'styleFingerprint':style,'contentProfile':profile,'performanceMemory':context.get('winningStrategies',[]),'localAI':local_ai_enabled(),'storage':media_store.backend_name(),'directorMode':mode,'editIntensity':intensity,'hookStyle':hook_style}
            update_job(jid,'running','director',20,f'V9 Director · mode {mode} · comparaison des stratégies',brief=brief)
            scores=[];total_revisions=0;last_strategy=''
            for variant in range(max(1,min(3,int(variants)))):
                _check_cancel(jid)
                plan,simulations=choose_plan(brief['project'],sources,style,profile,context,target,variant,0,mode,intensity,hook_style)
                ...
                director_label='VLM local + V9' if local_ai_enabled() else 'Director V9'
                ...
                initial=work/f'AutoDirector_V9_{variant+1}.mp4';render_plan(work,plan,paths,initial,captions,voice)
                _check_cancel(jid)
                ...
                if auto_revision and score<82 and MAX_REVISIONS>0:
                    update_job(jid,'running','revision',min(88,42+variant*18),f'V9 Critic · révision automatique · score {score}/100')
                    plan2,_=choose_plan(brief['project'],sources,style,profile,context,target,variant,1,mode,intensity,hook_style)
                    plan2,local_plan_diag2=refine_plan(brief['project'],plan2,sources,paths,work)
                    revised=work/f'AutoDirector_V9_{variant+1}_R1.mp4';render_plan(work,plan2,paths,revised,captions,voice)
                    _check_cancel(jid)
                ...
                update_job(jid,'running','render',min(94,55+variant*16),f'Variante {variant+1} terminée · {score}/100',score=max(scores),revision=total_revisions,strategy=last_strategy)
            with db() as c:c.execute("update jobs set status='done',stage='complete',progress=100,message='V9 terminé · galerie prête',output_asset_ids=%s,critic_score=%s,revision_count=%s,strategy=%s,updated_at=now() where id=%s",(output_ids,max(scores) if scores else 0,total_revisions,last_strategy,jid))
    except JobCancelled:
        _cleanup_outputs(output_ids);print('V9 JOB CANCELLED',jid,flush=True)
    except Exception as e:
        _cleanup_outputs(output_ids);print('V9 JOB FAILED',jid,repr(e),flush=True)
