...
def _cleanup_outputs(output_ids):
    ...
    with db() as c:
        rows=c.execute('select id,storage_key from assets where id=any(%s)',(output_ids,)).fetchall()
    for _,key in rows:
        if key:
...
def process_job(jid):
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix='autodirector_v8_') as td:
            ...
            update_job(jid,'running','director',20,'V8 Director: simulation de 5 stratégies',brief=brief)
            ...
            for variant in range(max(1,min(3,int(variants)))):
                ...
                if auto_revision and score<82 and MAX_REVISIONS>0:
                    update_job(jid,'running','revision',min(88,42+variant*18),f'V8 Critic: révision automatique, score {score}/100')
            ...
            with db() as c:c.execute("update jobs set status='done',stage='complete',progress=100,message='V8 terminé · galerie prête',output_asset_ids=%s,critic_score=%s,revision_count=%s,strategy=%s,updated_at=now() where id=%s",(output_ids,max(scores) if scores else 0,total_revisions,last_strategy,jid))
    except JobCancelled:
        _cleanup_outputs(output_ids)
        print('V8 JOB CANCELLED',jid,flush=True)
