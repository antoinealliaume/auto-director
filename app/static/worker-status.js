(()=>{
  ...
  function renderAgentControl(){
    ...
    if(agentState.needsUpdate){b.classList.add('install');b.textContent='↻ Mettre à jour le worker PC';s.textContent=`Agent PC ${agentState.version||'ancien'} : mise à jour requise`;return}
    ...
    b.textContent='▶ Démarrer le worker PC';s.textContent=`Agent PC ${agentState.version} : prêt · HTTPS`;
  }

  async function pollAgent(){
    ensureControls();
    try{const controller=new AbortController();const timer=setTimeout(()=>controller.abort(),1400);const r=await fetch(LOCAL_AGENT_URL+'/status',{cache:'no-store',mode:'cors',signal:controller.signal});clearTimeout(timer);if(!r.ok)throw new Error('agent offline');const data=await r.json();agentState.online=!!data.agent;agentState.workerRunning=!!data.workerRunning;agentState.version=String(data.agentVersion||'');agentState.needsUpdate=!agentState.version||Number.parseFloat(agentState.version)<2}
    catch{agentState.online=false;agentState.workerRunning=false;agentState.version='';agentState.needsUpdate=false}
    renderAgentControl();
  }
  ...
  function downloadInstaller(){
    ...
    notify('Téléchargement lancé. Ouvre INSTALL_AUTO_DIRECTOR_WORKER.bat une fois, puis reviens dans le Studio.');
  }
  ...
  async function handleLocalWorkerClick(){
    ...
    try{
      ...
      else{const session=localStorage.getItem('ad_token')||'';if(!session)throw new Error('Reconnecte-toi au Studio avant de lancer le worker PC.');await agentPost('/start',{studioUrl:location.origin,token:session});notify('Worker PC HTTPS en démarrage. Aucun secret PostgreSQL/Redis n’est envoyé au PC.')}
    }catch(e){notify(e.message||'Impossible de contrôler le worker PC.','error')}
    finally{agentState.busy=false;setTimeout(pollAgent,600);setTimeout(pollWorker,1400)}
  }
  ...
  function paintOffline(reason='Aucun heartbeat worker reçu.'){
    ...
    led.className='worker-led offline';label.textContent='Worker en attente';detail.textContent=reason+' Les jobs restent sauvegardés dans PostgreSQL et seront repris automatiquement.';if(stat)stat.textContent='Attente';if(profile)profile.textContent='Reprise auto';if(render)render.textContent='—';if(ai)ai.textContent='V8.6 sécurisé';if(queue)queue.textContent='File durable';
  }
