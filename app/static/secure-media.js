(()=>{
  ...
  renderGallery=function(){
    ...
    el.innerHTML=renders.map(a=>{const j=state.jobs.find(j=>(j.outputAssetIds||[]).includes(a.id));return `<article class="render-card" data-render-id="${a.id}"><div class="video-shell"><video controls preload="metadata" data-media="${a.id}"></video></div><div class="render-info"><h4>${esc(a.name)}</h4><div class="job-meta">${(a.size/1048576).toFixed(1)} Mo ${j?.criticScore!=null?`· <span class="score">${Number(j.criticScore).toFixed(1)}/100</span>`:''}</div><div class="render-actions"><a class="btn tiny ghost" data-view="${a.id}" href="#">Voir</a><a class="btn tiny secondary" data-download="${a.id}" href="#" download="${esc(a.name)}">Télécharger</a></div></div></article>`}).join('');
    renders.forEach(async a=>{try{const url=await mediaUrl(a.id),card=el.querySelector(`[data-render-id="${a.id}"]`);if(!card)return;const v=card.querySelector('video');if(v&&!v.src)v.src=url;const view=card.querySelector('[data-view]');if(view){view.href=url;view.target='_blank';view.rel='noopener'}const dl=card.querySelector('[data-download]');if(dl)dl.href=url}catch(e){console.warn('Media ticket failed',e)}});
  };
})();
