...
def _range(total, header):
    ...
    try:
        ...
        if not a:
            length=max(1,int(b));start=max(0,total-length);end=total-1
        else:
...
def attach(app):
    ...
    async def media(asset_id:str,request:Request,ticket:str=Query(...)):
        ...
        with db() as c:
            ...
            row=c.execute("select name,content_type,data,size,storage_key from assets where id=%s and kind='render'",(aid,)).fetchone()
        if not row:raise HTTPException(404,'Rendu introuvable')
        name,ctype,data,size,storage_key=row;total=int(size or 0)
        ...
        wanted=_range(total,request.headers.get('range',''))
        try:
            if wanted:
                start,end=wanted
                if storage_key:
                    part=media_store.get_range(storage_key,start,end)
                else:
                    blob=bytes(data or b'');part=blob[start:end+1]
            ...
            payload=media_store.read_asset(storage_key,data)
            headers['Content-Length']=str(len(payload))
            return Response(payload,media_type=ctype or 'video/mp4',headers=headers)
