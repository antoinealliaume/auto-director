...
DATABASE_URL=os.environ.get('DATABASE_URL','');BASE_URL=os.environ.get('TIKTOK_PUBLIC_BASE_URL','https://auto-director-web.onrender.com').rstrip('/');PULL_VERIFIED=os.environ.get('TIKTOK_PULL_URL_VERIFIED','0')=='1';PULL_TTL=max(900,min(3600,int(os.environ.get('TIKTOK_PULL_URL_TTL','3600'))))
...
def make_pull_token(asset_id):
    raw=f'{int(time.time())+PULL_TTL}.{asset_id}.{secrets.token_urlsafe(18)}';return raw+'.'+_sig(raw)
...
def _api_error(payload):
    err=(payload or {}).get('error') or {};code=str(err.get('code') or 'ok')
    if code!='ok':raise RuntimeError(str(err.get('message') or code)[:300])
...
def _title(caption,tags,cta):return _truncate_utf16('\n'.join(x for x in [str(caption or '').strip(),' '.join(str(x).strip() for x in (tags or []) if str(x).strip()),str(cta or '').strip()] if x),2200)
...
async def creator_info_call():
    ...
    async with httpx.AsyncClient(timeout=30) as c:r=await c.post('https://open.tiktokapis.com/v2/post/publish/creator_info/query/',headers={'Authorization':'Bearer '+access,'Content-Type':'application/json; charset=UTF-8'},json={})
    if r.status_code>=400:raise HTTPException(502,'TikTok creator_info indisponible')
    payload=r.json()
    try:_api_error(payload)
    except RuntimeError as exc:raise HTTPException(502,'TikTok API: '+str(exc)) from exc
    data=payload.get('data') or {}
    return {'username':data.get('creator_username') or '','nickname':data.get('creator_nickname') or '','avatarUrl':data.get('creator_avatar_url') or '','privacyLevels':data.get('privacy_level_options') or [],'commentDisabled':bool(data.get('comment_disabled')),'duetDisabled':bool(data.get('duet_disabled')),'stitchDisabled':bool(data.get('stitch_disabled')),'maxDurationSec':int(data.get('max_video_post_duration_sec') or 0)}

class DirectPostIn(BaseModel):
    privacyLevel:str;allowComment:bool=True;allowDuet:bool=True;allowStitch:bool=True;isAigc:bool=False;consent:bool=False

def _publication(publication_id):
    with db() as c:
        ensure_storage_schema(c);row=c.execute('''select q.id,q.asset_id,q.caption,q.hashtags,q.cta,q.status,a.name,a.content_type,a.data,a.size,a.storage_key,a.metadata from publications q join assets a on a.id=q.asset_id where q.id=%s and a.kind='render' ''',(publication_id,)).fetchone()
...
def _mark_failed(qid,error):
    try:
        with db() as c:c.execute("update publications set status='failed',error=%s,updated_at=now() where id=%s",(str(error)[:500],qid))
    except Exception:pass

def attach(app):
    async def readiness(authorization:Optional[str]=Header(None)):
        require_studio(authorization);s=connection_status();return {**s,'pullUrlVerified':PULL_VERIFIED,'pullUrlPrefix':BASE_URL+'/api/tiktok/pull/','directPostOperational':bool(s.get('directPostReady') and PULL_VERIFIED),'scheduledAutoSend':False}
    ...
    async def pull_media(asset_id:str,request:Request,token:str=Query(...)):
        ...
        with db() as c:
            ensure_storage_schema(c);row=c.execute("select content_type,data,size,storage_key from assets where id=%s and kind='render'",(aid,)).fetchone()
        if not row:raise HTTPException(404,'Media introuvable')
        ctype,data,size,key=row;total=int(size or 0);headers={'Content-Length':str(total),'Cache-Control':'private, no-store','X-Content-Type-Options':'nosniff','Accept-Ranges':'bytes'}
        if request.method=='HEAD':return Response(status_code=200,media_type=ctype or 'video/mp4',headers=headers)
        return StreamingResponse(_stream_asset(key,data,total),media_type=ctype or 'video/mp4',headers=headers)
    async def direct_post(publication_id:str,x:DirectPostIn,authorization:Optional[str]=Header(None)):
        ...
        if not connection_status().get('directPostReady'):raise HTTPException(409,'TikTok video.publish non autorisé.')
        ...
        with db() as c:
            claimed=c.execute("update publications set status='initializing',error='',updated_at=now() where id=%s and status in ('ready','scheduled','failed') returning id",(qid,)).fetchone()
            if not claimed:
                state=c.execute('select status from publications where id=%s',(qid,)).fetchone();current=state[0] if state else 'missing'
                raise HTTPException(409,'Cette publication est déjà en cours ou terminée.' if current in {'initializing','publishing','published'} else 'Cette publication ne peut pas être envoyée.')
        try:
            title=_title(caption,tags,cta);access=get_access_token();video_url=pull_url(aid);body={'post_info':{'title':title,'privacy_level':x.privacyLevel,'disable_duet':not x.allowDuet,'disable_comment':not x.allowComment,'disable_stitch':not x.allowStitch,'is_aigc':bool(x.isAigc)},'source_info':{'source':'PULL_FROM_URL','video_url':video_url}}
            async with httpx.AsyncClient(timeout=45) as client:r=await client.post('https://open.tiktokapis.com/v2/post/publish/video/init/',headers={'Authorization':'Bearer '+access,'Content-Type':'application/json; charset=UTF-8'},json=body)
            if r.status_code>=400:raise RuntimeError('TikTok Direct Post a refusé l’initialisation.')
            payload=r.json();_api_error(payload);publish_id=str((payload.get('data') or {}).get('publish_id') or '')
            if not publish_id:raise RuntimeError('TikTok n’a pas renvoyé de publish_id.')
        except Exception as exc:
            _mark_failed(qid,exc);raise HTTPException(502,str(exc)[:300]) from exc
    ...
    async def fetch_status(publication_id:str,authorization:Optional[str]=Header(None)):
        ...
        async with httpx.AsyncClient(timeout=30) as client:r=await client.post('https://open.tiktokapis.com/v2/post/publish/status/fetch/',headers={'Authorization':'Bearer '+access,'Content-Type':'application/json; charset=UTF-8'},json={'publish_id':publish_id})
        if r.status_code>=400:raise HTTPException(502,'Statut TikTok indisponible')
        payload=r.json()
        try:_api_error(payload)
        except RuntimeError as exc:raise HTTPException(502,'TikTok API: '+str(exc)) from exc
        data=payload.get('data') or {};remote=str(data.get('status') or '');local='publishing';fail=''
    ...
    app.add_api_route('/api/tiktok/readiness',readiness,methods=['GET'],include_in_schema=False);app.add_api_route('/api/tiktok/creator-info',creator_info,methods=['GET'],include_in_schema=False);app.add_api_route('/api/tiktok/pull/{asset_id}',pull_media,methods=['GET','HEAD'],include_in_schema=False);app.add_api_route('/api/tiktok/direct-post/{publication_id}',direct_post,methods=['POST'],include_in_schema=False);app.add_api_route('/api/tiktok/post-status/{publication_id}',fetch_status,methods=['POST'],include_in_schema=False)
