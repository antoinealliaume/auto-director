...
DATABASE_URL=os.environ.get('DATABASE_URL','');REDIS_URL=os.environ.get('REDIS_URL','')
CLIENT_KEY=os.environ.get('TIKTOK_CLIENT_KEY','').strip();CLIENT_SECRET=os.environ.get('TIKTOK_CLIENT_SECRET','').strip();REDIRECT_URI=os.environ.get('TIKTOK_REDIRECT_URI','https://auto-director-web.onrender.com/api/tiktok/callback').strip();SCOPES=os.environ.get('TIKTOK_SCOPES','user.info.basic,video.upload,video.publish').strip();OWNER='studio_owner';STATE_TTL=600
...
def ensure_schema():
    with db() as c:c.execute('''create table if not exists tiktok_connections(id text primary key,open_id text not null default '',access_token_enc text not null,refresh_token_enc text not null,scopes text not null default '',expires_at timestamptz not null,refresh_expires_at timestamptz not null,created_at timestamptz not null default now(),updated_at timestamptz not null default now())''')
...
def consume_state(state):
    ...
    try:pipe=r.pipeline();pipe.get(key);pipe.delete(key);value,_=pipe.execute();return value=='1'
...
def _save_tokens(data):
    ...
    now=datetime.now(timezone.utc);expires=now+timedelta(seconds=max(60,int(data.get('expires_in') or 86400)));refresh_exp=now+timedelta(seconds=max(3600,int(data.get('refresh_expires_in') or 31536000)));ensure_schema()
    with db() as c:c.execute('''insert into tiktok_connections(id,open_id,access_token_enc,refresh_token_enc,scopes,expires_at,refresh_expires_at) values(%s,%s,%s,%s,%s,%s,%s) on conflict(id) do update set open_id=excluded.open_id,access_token_enc=excluded.access_token_enc,refresh_token_enc=excluded.refresh_token_enc,scopes=excluded.scopes,expires_at=excluded.expires_at,refresh_expires_at=excluded.refresh_expires_at,updated_at=now()''',(OWNER,str(data.get('open_id') or ''),encrypt(access),encrypt(refresh),str(data.get('scope') or ''),expires,refresh_exp))
...
def connection_status():
    ...
    scopes=[x.strip() for x in (row[1] or '').split(',') if x.strip()];now=datetime.now(timezone.utc);connected=row[3]>now
    return {'configured':True,'connected':connected,'scopes':scopes,'directPostReady':'video.publish' in scopes and connected,'uploadReady':'video.upload' in scopes and connected,'expiresAt':row[2].isoformat(),'refreshExpiresAt':row[3].isoformat(),'openIdSuffix':(row[0][-6:] if row[0] else '')}
...
def get_access_token():
    ...
    with httpx.Client(timeout=30) as c:r=c.post('https://open.tiktokapis.com/v2/oauth/token/',data={'client_key':CLIENT_KEY,'client_secret':CLIENT_SECRET,'grant_type':'refresh_token','refresh_token':refresh},headers={'Content-Type':'application/x-www-form-urlencoded','Cache-Control':'no-cache'});r.raise_for_status();data=r.json()
...
def attach(app):
    ...
    async def connect(authorization:Optional[str]=Header(None)):
        ...
        state=create_state();params={'client_key':CLIENT_KEY,'response_type':'code','scope':SCOPES,'redirect_uri':REDIRECT_URI,'state':state};return {'authorizationUrl':'https://www.tiktok.com/v2/auth/authorize/?'+urlencode(params),'expiresIn':STATE_TTL}
    async def callback(code:Optional[str]=Query(None),state:Optional[str]=Query(None),error:Optional[str]=Query(None),error_description:Optional[str]=Query(None)):
        if error:
            detail=html.escape(str(error_description or error)[:300]);return HTMLResponse(f'<!doctype html><meta charset="utf-8"><title>TikTok</title><h2>Connexion TikTok refusée</h2><p>{detail}</p><p>Tu peux fermer cette page.</p>',status_code=400)
        ...
        try:
            async with httpx.AsyncClient(timeout=30) as c:r=await c.post('https://open.tiktokapis.com/v2/oauth/token/',data={'client_key':CLIENT_KEY,'client_secret':CLIENT_SECRET,'code':code,'grant_type':'authorization_code','redirect_uri':REDIRECT_URI},headers={'Content-Type':'application/x-www-form-urlencoded','Cache-Control':'no-cache'});r.raise_for_status();data=r.json()
            if data.get('error'):raise RuntimeError(str(data.get('error_description') or data['error']))
            _save_tokens(data)
        except Exception as exc:
            detail=html.escape(str(exc)[:300]);return HTMLResponse(f'<!doctype html><meta charset="utf-8"><title>TikTok</title><h2>Connexion TikTok impossible.</h2><p>{detail}</p>',status_code=502)
    ...
    async def disconnect(authorization:Optional[str]=Header(None)):
        ...
        return {'ok':True}
    app.add_api_route('/api/tiktok/status',status,methods=['GET'],include_in_schema=False);app.add_api_route('/api/tiktok/connect',connect,methods=['POST'],include_in_schema=False);app.add_api_route('/api/tiktok/callback',callback,methods=['GET'],include_in_schema=False);app.add_api_route('/api/tiktok/disconnect',disconnect,methods=['POST'],include_in_schema=False)
