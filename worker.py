import asyncio,os,subprocess,tempfile,uuid,threading,json
...
def render_job(jid:str):
    try:
        ...
        with tempfile.TemporaryDirectory(prefix='autodirector_') as td:
            ...
            for v in range(max(1,min(3,variants))):
                ...
                segs=[];order=sources[v:]+sources[:v]
                ...
                title=(project[0] if project else 'Auto Director')[:90];voice=td/f'v{v}.mp3'
    ...
    except Exception as e:update(jid,'failed','error',0,str(e)[:500])

class Health(BaseHTTPRequestHandler):
    def do_GET(self):
        if self.path!='/health':self.send_response(404);self.end_headers();return
        body=json.dumps({'ok':True,'worker':'ready'}).encode();self.send_response(200);self.send_header('Content-Type','application/json');self.send_header('Content-Length',str(len(body)));self.end_headers();self.wfile.write(body)
